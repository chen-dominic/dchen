...
export default function Experience(){
    return (
        <div className="flex flex-col gap-4 my-8 text-lightText items-center" id="Experience">
            <div className="flex-1 md:mx-4 mx-0 bg-offPrimary md:py-10 md:px-14 p-4 rounded-3xl transition-all group md:w-[60%]">
                <div className="flex">
                    ...
                    <div className="flex flex-col ml-4">
                        <h1 className="text-white font-bold text-xl pb-4 uppercase group-hover:text-secondary transition-all md:block hidden">Work</h1>
                        {experiences.professional.map((item, index) => (
                            <div className="text-lightText md:py-2 py-0 group/child mb-8" key={index}>
                                ...
                                <h1 className="text-white text-2xl font-bold group-hover/child:text-secondary transition-all">{item.title}</h1>
                                <h1 className="text-lg font-semibold">{item.location}</h1>
                                {item.description.map((desc, index) => (
                                    <h1 className="font-light text-base" key={index}>• {desc}</h1>
                                ))}
                            </div>
                        ))}
                    </div>
